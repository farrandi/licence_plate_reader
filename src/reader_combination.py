...
class LicenseReader():
    ...
    def findPlate(self, cImage):
        ...
        img = cv2.cvtColor(cameraImage, cv2.COLOR_BGR2HSV) 

        #cv2.drawContours(cameraImage, [screenCnt], -1, (0, 0, 255), 3)
        blue_mask = cv2.inRange(img, np.array([120,122,90],np.uint8), np.array([120,255,204],np.uint8))
        new_image = cv2.bitwise_and(cameraImage, cameraImage,mask=blue_mask)
        ...
        (x, y) = np.where(blue_mask == 255)
        (topx, topy) = (np.min(x), np.min(y))
        (bottomx, bottomy) = (np.max(x), np.max(y))
        cropped = img[topx+10:bottomx, topy:bottomy/2]

        
        try:
            gray_mask = cv2.inRange(cropped, np.array([0,0,90],np.uint8), np.array([0,0,210],np.uint8))
            cv2.imshow('gray', gray_mask)
            cv2.waitKey(3)
            final_image = cv2.bitwise_and(cropped, cropped, mask=gray_mask)
            (x2, y2) = np.where(gray_mask == 255)
            (topx2, topy2) = (np.min(x2), np.min(y2))
            (bottomx2, bottomy2) = (np.max(x2), np.max(y2))
            final_cropped = cropped[topx2:bottomx2, topy:bottomy2+1]
            final_cropped = cv2.cvtColor(final_cropped, cv2.COLOR_HSV2BGR)
